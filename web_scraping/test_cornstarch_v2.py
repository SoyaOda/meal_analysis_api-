#!/usr/bin/env python3
"""
Cornstarch serving情報取得の詳細デバッグテスト（修正版コレクター使用）
"""
import sys
import os
import asyncio
...
async def main():
    print("🧪 Cornstarch Serving情報取得テスト（修正版コレクター）")
    print("=" * 60)
    ...
    try:
        ...
        # 結果を保存
        output_file = f"debug/cornstarch_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        # スクリーンショット保存
        screenshot_file = f"debug/cornstarch_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
