...
【v2】: Macaroni cooked enrichedを手動で追加
...
4. マッピングが見つからない食材を除外
5. **NEW**: Macaroni cooked enrichedを追加
...
def merge_nutrition_data(
    ...
    stats = {
        ...
        'manually_added_macaroni': 0,
        'excluded_items': []
    }

    # 最大IDを追跡（新しいmacaroniエントリ用）
    max_id = max([item['id'] for item in stemmed_data], default=10000000000)
    ...
    # ===== NEW: Macaroni cooked enrichedを手動で追加 =====
    print("\n【Macaroni cooked enrichedを追加中】")
    ...
    macaroni_cooked_entry = {
        ...
        "data_type": "unified",
        "source": "MyNetDiary",
        "processing_method": "manual_addition_v2",
        "conversion_timestamp": datetime.now().timestamp(),
    ...
    merged_data.append(macaroni_cooked_entry)
    stats['manually_added_macaroni'] = 1
    print(f"✓ Macaroni cooked enriched added with ID: {max_id + 1}")
...
def save_merge_report(stats: dict, filepath: Path):
    ...
    report = {
        ...
        'summary': {
            ...
            'manually_added_macaroni': stats['manually_added_macaroni'],
...
def print_summary(stats: dict):
    """サマリーを表示"""
    print("=" * 80)
    print("Stemmed DB栄養情報マージレポート（v2 with Macaroni）")
    print("=" * 80)
    print(f"\n【統計】")
    ...
    print(f"Macaroni手動追加: {stats['manually_added_macaroni']}件")
    print(f"\n【除外】")
