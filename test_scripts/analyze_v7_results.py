#!/usr/bin/env python3
"""Analyze v7_production test results"""

import json
import sys

# Load JSON data
with open('test_scripts/output/vlm_prompt_nutrition_comparison_20251027_151747.json', 'r') as f:
    data = json.load(f)

print("=" * 80)
print("📊 v5_streamlined vs v7_production 比較結果 (3画像)")
print("=" * 80)

# 各プロンプトの誤差を集計
results = {"v5_streamlined": [], "v7_production": []}

for img in data:
    img_name = img["image_name"]
    for prompt in ["v5_streamlined", "v7_production"]:
...
v5_avg = sum(e["cal_error"] for e in results["v5_streamlined"]) / len(results["v5_streamlined"])
v7_avg = sum(e["cal_error"] for e in results["v7_production"]) / len(results["v7_production"])

if v7_avg < v5_avg:
    improvement = v5_avg - v7_avg
    print(f"✅ v7_production が v5_streamlined より {improvement:.1f}% 改善")
else:
    degradation = v7_avg - v5_avg
    print(f"⚠️  v7_production が v5_streamlined より {degradation:.1f}% 悪化")
    print(f"   → 小物検出の強化が期待通りに機能していない可能性")
