...
class ResultManager:
    ...
    def _create_analysis_summary(self) -> dict:
        ...
        # phase_results から各フェーズのデータを取得（正しいキー名で）
        phase1_data = self.phase_results.get("phase1", {})
        nutrition_calculation_data = self.phase_results.get("nutrition_calculation", {})
        ...
        summary_data = {
            ...
            "ai_model_used": phase1_data.get("metadata", {}).get("ai_model_used", phase1_data.get("input_data", {}).get("llm_model_id", "unknown")),
    ...
    def _create_detailed_analysis(self) -> dict:
        ...
        # phase_results から各フェーズのデータを取得（正しいキー名で）
        phase1_data = self.phase_results.get("phase1", {})
        nutrition_search_data = self.phase_results.get("nutrition_search", {})
        nutrition_calculation_data = self.phase_results.get("nutrition_calculation", {})
    ...
    def finalize_pipeline(self, final_result: dict = None):
        """パイプラインの終了処理：3つのファイルを作成して保存"""
        try:
            # final_result が渡された場合は、それをインスタンス変数に保存
            if final_result:
                self.final_result = final_result
                
                # final_resultからphase_resultsを更新
                if 'phase1_result' in final_result:
                    self.phase_results['phase1'] = final_result['phase1_result']
                if 'nutrition_search_result' in final_result:
                    self.phase_results['nutrition_search'] = final_result['nutrition_search_result']
                if 'final_nutrition_result' in final_result:
                    self.phase_results['nutrition_calculation'] = final_result['final_nutrition_result']
            ...
            print(f"❌ ファイル保存エラー: {e}")
            raise  # 新しい構造では特別な終了処理は不要  # 新しい構造では特別な終了処理は不要  # 新しい構造では特別な終了処理は不要  # 新しい構造では特別な終了処理は不要
