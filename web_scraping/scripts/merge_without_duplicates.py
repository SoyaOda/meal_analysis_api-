#!/usr/bin/env python3
"""
重複を除いてprocessed foodsを統合するスクリプト
"""
...
def merge_without_duplicates(main_file: str, manual_file: str, output_dir: str) -> tuple:
    """重複を除いて2つのprocessed foodsファイルを統合"""
    ...
    # Web収集食材の名前セット（正規化）
    web_food_names = {food['food_name'].lower().strip() for food in main_data['foods']}
    ...
    for food in manual_data['foods']:
        ...
        if normalized_name in web_food_names:
            duplicates.append(food['food_name'])
        else:
            manual_foods_unique.append(food)

    # 統合（Web収集 + 重複除外後の手作業）
    merged_foods = main_data['foods'] + manual_foods_unique
        ...
        'metadata': {
            ...
            'sources': [
                ...
                },
                {
                    ...
                    'original_count': len(manual_data['foods']),
                    'duplicates_removed': len(duplicates),
                    'unique_count': len(manual_foods_unique)
                }
            ],
            'processing_notes': 'Merged web-scraped and manually entered foods (duplicates removed)',
    ...
    output_file = os.path.join(output_dir, f"all_foods_processed_deduped_{timestamp}.json")
    ...
    summary_data = {
        ...
        'deduplication_summary': {
            'total_manual_foods': len(manual_data['foods']),
            'duplicates_removed': len(duplicates),
            'unique_manual_foods': len(manual_foods_unique),
            'duplicate_names': duplicates
        }
    }

    summary_file = os.path.join(output_dir, f"all_foods_summary_deduped_{timestamp}.json")
    with open(summary_file, 'w', encoding='utf-8') as f:
        json.dump(summary_data, f, ensure_ascii=False, indent=2)

    return output_file, summary_file, category_stats, duplicates
...
def main():
    ...
    print("🔄 Processed Foodsの統合（重複除外版）")
    print("=" * 80)
    print(f"📁 メインファイル: {main_file.name}")
    print(f"📁 手作業ファイル: {manual_file.name}")

    # 統合実行
    output_file, summary_file, category_stats, duplicates = merge_without_duplicates(
        str(main_file),
        str(manual_file),
        str(processed_dir)
    )
    ...
    for source in merged_data['metadata']['sources']:
        ...
        if source['type'] == 'web_scraping':
            ...
            print(f"   元の食材数: {source['original_count']}個")
            print(f"   重複除外: {source['duplicates_removed']}個")
            print(f"   ユニーク食材: {source['unique_count']}個")

    print(f"\n✅ 合計食材数（重複なし）: {merged_data['metadata']['total_foods']}個")

    # 重複リスト表示
    if duplicates:
        print(f"\n⚠️  除外された重複食材 ({len(duplicates)}個):")
