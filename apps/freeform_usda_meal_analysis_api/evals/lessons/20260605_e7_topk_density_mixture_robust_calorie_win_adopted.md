...
## Tail follow-up (2026-06-05) — selective-E7 does NOT work; tail is intrinsic
Per-image isolation on the frozen-VLM runs (top1 vs E7 share the SAME VLM output ⇒ delta = pure E7 effect) quantified the mean/tail split: E7 cuts MAE on all 3 sets (−2.13/−7.67/−1.61) but **fattens p90 on the hard sets** (NVReal 84→90, **N5k 112→134**; frozen-50 tail improves 34→30). The worst E7 regressions concentrate on **single/dominant-item meals** (n_pred==1, max_cal_share→1.0; e.g. NVReal test_food15 +56.9pt, N5k test_food67 +171.9pt) — one bad neighbour density with no cross-item averaging to dampen it.
**Counterfactual (free): meal-level selective rules fail.** Diverting single/dominant-item meals to top-1 (`top1 if n_pred==1` / `share>=0.85`) **worsened pooled MAE (44.4→45.3) and did NOT cut p90** — across the single-item *population* E7 still wins on average; only a few are catastrophic, and they're not separable by meal features. **pure-top1 has the lowest pooled p90 (91.6 vs 94.1) but worst MAE (48.6)** ⇒ E7's mean-win and fatter tail are two sides of one mechanism (density-smoothing). No simple selection separates them; only τ traces the top1(τ→0)↔E7(τ=1) frontier. Tooling: `/tmp/e7_tail_analysis.py`, `e7_selective_counterfactual.py`.

## Follow-ups
- Confirm on pooled N5k(250)+NVReal(104) (the wall-distribution + eye-level realistic) before deploy. 50-set is eye-level-realistic but small. **[DONE 2026-06-05: pooled win holds; see tail follow-up above.]**
- Sweep τ (E7_DENSITY_TEMP) and k=8/10 — the ONLY remaining E7-tail lever (selective ruled out). Needs τ as a per-candidate override (post-VLM, frozen-cache) → one clean run can test whether τ<1 is Pareto-better (smaller tail) or just frontier-interpolation. Paid (1 frozen VLM pass + retrieval-only τ sweep).
