...
**現在地**: pro を採用候補に決定(コード既定/doc 更新済, 本番未変更)。**🔴重要: frozen-50 GT は GPT-5-pro推定と判明→calorie系数値は実精度でなく一致度。両 calibration JSON は VOID, follow-up#1(bias補正)停止。TWO-GATE採用(N5k実測カロリー＋50画像)**。Nutrition5k 250枚を**2 run 評価済**: pro の calorie 優位は方向的に一貫(~7pt 低)だが**有意でなく BORDERLINE**(denoised CI[-15.2,+0.9] p=0.076; run1単独の「有意」は楽観draw)、かつ **pro は run間再現性が flash より低い**(identical率20% vs 52%)。cal_MAE ~58-62%=50の~18%は過大評価。**次=実ドメイン実測アンカー構築/NutritionVerse-Real取得(Codex指示書済)/recognition多様性(ISIA等)**。
...
**直近の数値（pro vs flash, 同v13）**: recognition F1 pro>flash 4/4(決定的, 信頼), latency 同等(+0.2s), cost ~3.2x。**calorie: frozen-50 ~18%は GPT-5-pro一致度(実精度でない); 独立実測 Nutrition5k 250枚を2 run — run1 flash 68.4/pro 58.4, run2 flash 66.4/pro 61.9。denoised gap -7.2pt だが paired CI[-15.2,+0.9] p=0.076=BORDERLINE(有意でない)。pro は run間再現性が低い(identical率 20% vs flash 52%)**。conviction/naming は judge(advisory)。**OpenRouter は補充済**。
