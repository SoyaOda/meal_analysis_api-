...
class ModelConfig(BaseModel):
    ...
    vlm_model_id: Optional[str] = Field(
        None,
        description="VLMモデルID（例: 'openrouter:google/gemini-3-flash-preview'）。指定しない場合はAdmin Panel設定を使用",
        example="openrouter:google/gemini-3-flash-preview",
    )
    ...
    temperature: Optional[float] = Field(
        None, description="生成温度（0.0-2.0）", ge=0.0, le=2.0, example=0.3
    )

    seed: Optional[int] = Field(
        None,
        description="生成シード（再現性向上用）",
        ge=0,
        le=2147483647,
        example=42,
    )
    ...
    max_tokens: Optional[int] = Field(
        None, description="最大トークン数", ge=1, le=32768, example=12288
    )

    use_vlm_cache: Optional[bool] = Field(
        None,
        description="VLMキャッシュ利用可否。未指定時はサーバー設定を使用",
        example=False,
    )
...
class SearchConfig(BaseModel):
    ...
    stage1_top_k: Optional[int] = Field(
        None, description="Stage1で取得する候補数", ge=1, le=200, example=50
    )

    # ========== Hybrid Search パラメータ ==========
    bm25_weight: Optional[float] = Field(
        None, description="BM25検索のスコア重み（0.0-1.0）", ge=0.0, le=1.0, example=0.4
    )
...
class ImageAnalysisRequest(BaseModel):
    ...
    model_config_override: Optional[ModelConfig] = Field(
        None, description="モデル設定のオーバーライド"
    )

    search_config_override: Optional[SearchConfig] = Field(
        None, description="検索設定のオーバーライド"
    )
...
class VoiceAnalysisRequest(BaseModel):
    ...
    user_context: Optional[str] = Field(
        None, description="ユーザーコンテキスト", example="voice lunch analysis"
    )

    language_code: str = Field(
        "en-US", description="音声認識言語コード", example="ja-JP"
    )
