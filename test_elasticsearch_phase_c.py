...
Local DBベースのElasticsearchとの統合による実際のAPIパイプラインテスト
"""
import asyncio
import json
import logging
...
async def test_phase_c_enhanced_integration():
    ...
    try:
        ...
        from app_v2.models.nutrition_search_models import NutritionQueryInput, NutritionQueryOutput
        ...
        # 基本的なクエリテスト（Local DB用インターフェース使用）
        basic_query = NutritionQueryInput(
            ingredient_names=["chicken breast"],
            dish_names=[],
            preferred_source="local_database"
        )
        
        # USDA互換レイヤー経由ではなく、直接NutritionQueryを処理
        basic_result = await es_component._process_nutrition_search(basic_query)
        ...
        cook_query = NutritionQueryInput(
            ingredient_names=["cook"],
            dish_names=[],
            preferred_source="local_database"
        )
        
        cook_result = await es_component._process_nutrition_search(cook_query)
        ...
        for typo_ingredients, expected in typo_test_cases:
            typo_query = NutritionQueryInput(
                ingredient_names=typo_ingredients,
                dish_names=[],
                preferred_source="local_database"
            )
            
            typo_result = await es_component._process_nutrition_search(typo_query)
        ...
        for synonym_ingredients, expected in synonym_test_cases:
            synonym_query = NutritionQueryInput(
                ingredient_names=synonym_ingredients,
                dish_names=[],
                preferred_source="local_database"
            )
            
            synonym_result = await es_component._process_nutrition_search(synonym_query)
        ...
        # 鶏肉検索（高タンパク、低脂肪を期待）
        chicken_query = NutritionQueryInput(
            ingredient_names=["chicken breast grilled"],
            dish_names=[],
            preferred_source="local_database"
        )
        
        chicken_result = await es_component._process_nutrition_search(chicken_query)
        ...
        for test_case in test_cases:
            query = NutritionQueryInput(
                ingredient_names=test_case["query"],
                dish_names=[],
                preferred_source="local_database"
            )
            
            result = await es_component._process_nutrition_search(query)
            ...
            if len(result.matches) > 0:
                ...
                calories = 0
                protein = 0
                for nutrient in top_match.nutrients:
                    if "energ" in nutrient.name.lower() or "calorie" in nutrient.name.lower():
                        calories = nutrient.amount
                    elif "protein" in nutrient.name.lower():
                        protein = nutrient.amount
        ...
        # 8. Local DB Elasticsearch検索精度サマリー
        ...
        # データソース別サマリーを表示
        source_summary = basic_result.get_source_summary()
        print(f"   データソース別マッチ数:")
        ...
        print("📌 システム: Local DBベースElasticsearch検索")
