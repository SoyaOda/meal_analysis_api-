...
1. [Overview](#overview)
2. [Step 1: VLM Prompt](#step-1-vlm-prompt)
3. [Step 2: VLM Results](#step-2-vlm-results)
4. [Step 3: Query Generation](#step-3-query-generation)
5. [Step 4: Search Pipeline Results](#step-4-search-pipeline-results)
   - [Stage 1: Retriever](#stage-1-retriever)
   - [Stage 2: Reranker](#stage-2-reranker)
6. [Summary](#summary)
...
```
画像 → VLM (Vision Language Model) → クエリ生成 → 検索パイプライン → 結果
                                                  ├─ Stage 1: Retriever (FAISS + Embedding)
                                                  └─ Stage 2: Reranker (LLM-based)
```
