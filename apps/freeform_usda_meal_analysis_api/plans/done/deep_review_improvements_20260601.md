...
- **self-verification 2nd pass 実装＆検証→same-tier verifier非有効(機能はOK)**: `vlm_service.verify_items`/`apply_verification`(not_present除去, unsure保持=recall安全, 名前不一致は保持=fail-safe)＋pipeline Step1.5(`enable_self_verification` flag)＋router form field＋harness(build_candidate_listのwhitelistバグも修正)＋verify prompt。4 tests。**dev40 A/B(生成cache共有, verifyのみ変数)**: verify は **0.17 item/img しか落とせず**(同一gemini-3-flash verifierが phantom大半を追認)、per-dish hallucinated 58→51・recognition 2.175→2.237 だが **conviction横ばい(32.08→32.11)**、missed微増。**研究のprecision最大レバーも同一安価モデルでは中和**。実装は保持(opt-in default OFF, 上位verifier実験の土台)。lesson: `20260602_self_verification_implemented_same_tier_verifier_ineffective.md`。
- **メタ結論(堅牢)**: reranker/v16/v17/self-verification 全てで **conviction ~32天井を超えず**=gemini-3-flash の認識capability-bound。非marginalな改善には**上位GENERATORモデル(gemini-3.1-pro, portion slope最良)**が現実解。inference-time patch では頭打ち。
- **次options(要判断)**: (1)上位verifier(gemini-3.1-pro)でself-verify再試行 (2)gemini-3.1-pro を generator として認識+slopeで評価 (3)v17を50枚full→promote判断。
...
| 2026-06-02 | deep-review | **self-verification 2nd pass 実装**(`verify_items`/`apply_verification`/pipeline Step1.5/router/harness, +4 tests, opt-in default OFF)＋harness whitelistバグ修正。dev40 A/B: 同一gemini-3-flash verifierは **0.17item/img しか落とせず**(phantom大半追認)、hallucinated 58→51・recognition微増だが **conviction横ばい32.08→32.11**。**メタ: reranker/v16/v17/self-verify 全てconviction ~32天井超えず=capability-bound, 上位generatorが現実解**。実装保持(上位verifier実験の土台)。lesson: self_verification_implemented_same_tier。 |

### 2026-06-03: 上位generator検証 ＋ v17 promote判定 ＋ OpenRouter credit枯渇
- **🟢 gemini-3.1-pro が認識天井を破る（決定的・課金不要）**: pro vs flash(同v13, dev40, run 20260602_235020) で **recog_F1 0.204→0.266(+30%相対)・recall 0.219→0.291・precision 0.197→0.252 全向上**、cal_MAE 18.21→17.75・high30 17.5→12.5。**inference-timeレバーが全てnear-noiseだったのに対し、上位generatorは recognition を notably 改善**＝メタ結論を実証。LLM-judge確認は credit枯渇でブロック中（出力は保存済, top-up後に再judge数分）。lesson: `20260603_gemini31pro_breaks_recognition_ceiling_deterministic.md`。
- **v17 promote → 却下（full50確定）**: cal_MAE 18.14→**22.78(+4.6pt悪化)**(cooked-default が高密度寄り), recognition F1 微増(0.238→0.251 noise域)。near-noiseのrecognition gain に対しcalorie回帰が大→**v13維持**。describe-shape単体(cooked-default無し)は将来候補だがpro upgradeが上位。lesson: `20260603_v17_full50_rejected_calorie_regression.md`。
...
### 2026-06-03 続2: pro stability(4 run)＋calorie bias対処 → HOLD条件を実質クリア
- **stability(4独立run: dev40×3+full50, fresh VLM)**: **recog_F1 pro>flash 4/4** ([0.062,0.016,0.031,0.019])、**conviction pro>flash 4/4** (+1.93/+1.52/+2.39/+2.10, mean+1.99, 各runはCI重複でNSだが向き一貫)。correct% 3/4。**naming pro<flash 4/4** (-0.10〜-0.15, 一貫した小回帰=要注意)。→ recognition改善は**再現性確立**。
- **calorie bias対処(calibration, fit dev40→test holdout10 disjoint)**: pro 過小バイアス **-13.5%→-1.6%補正**、**pro+calib 13.98% > flash+calib 15.61%**(held-out n=10は小だがbias補正は確実)。config: `evals/calorie_calibration_pro_v13.json`(enabled=false)。
- **更新評価**: HOLD条件(stability未検証/calorie under-bias)を**実質クリア**。pro は **validated recognition + calibrated-calorie改善**で conviction も4/4で+方向。残る難点: naming一貫回帰(-0.125)・各conviction NS・4xコスト。**→ calibration ON前提なら防御可能なpromote候補(4xコスト受容が条件, 本番switchはdeploy判断)**。naming回帰は pro + (修正済)reranker instruction で回復可能性(cheap follow-up)。lesson: `20260603_pro_stability_confirmed_and_calorie_bias_calibrated.md`。
| 2026-06-03 | deep-review | **pro stability(4run)＋calorie bias対処**: **recog_F1 4/4・conviction 4/4(+1.99mean)・naming 4/4回帰(-0.125)**で recognition再現性確立。calibration(disjoint)で pro bias -13.5%→-1.6%補正・**pro+calib 13.98%>flash+calib 15.61%**。→ HOLD条件(stability/bias)実質クリア=**calibration ON前提で防御可能なpromote候補**(4xコスト受容＋deploy判断)。残: naming回帰・各NS。lesson: pro_stability_confirmed_and_calorie_bias_calibrated。 |
