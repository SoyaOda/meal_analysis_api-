...
このAPIは写真から食事を分析し、栄養情報を計算するシステムです。Deep Infra AI (Qwen2.5-VL-32B-Instruct等) とElasticsearchを使用した高精度な食事分析を提供します。
...
**注意**: Cloud Run環境ではElasticsearchが利用できないため、`USE_ELASTICSEARCH_SEARCH=false`に設定し、フォールバック検索を使用します。
