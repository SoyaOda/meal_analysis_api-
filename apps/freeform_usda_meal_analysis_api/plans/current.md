...
- **次の本筋(要サインオフ＝conviction KPI構成変更)**: (A) portion_plausibilityを決定的 `portion_bands` スコアでゲートし、LLM judgeのportionはadvisory化(overall_conviction幾何平均の0.20重みから除外/減衰)。LLM judgeは認識/命名/nutrient-macro/convictionのみ担当。(B) `_default_pred_gt_similarity` を token/char→embedding に格上げ(portion・naming・recall共通レバー、**新規依存→要確認**)。(C) ユーザがドラフト(`golden_set.draft_claude_v2.jsonl`)をレビュー→`golden_set.jsonl`確定→人手κ。naming/conviction/nutrientは相対A/Bに使用可。
