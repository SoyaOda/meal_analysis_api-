...
> - **pro 採用は維持するが、根拠は recognition のみに縮小**（決定的 F1 4/4）。**calorie の優位は 3 独立セットで非頑健と判明**（2026-06-04）: frozen-50(GPT-5-pro推定,eye-level)=pro≈flash NS／N5k(俯瞰,実測)=pro −7pt borderline CI[-15.2,+0.9]／**NutritionVerse-Real(eye-level,実測,104 content-verified)=flash がわずか良 +2.2pt NS**。**pro−flash の符号が flip＝pro に頑健な calorie 優位は無い**。さらに pro は run間再現性が flash より低い(bit-identical 20% vs 52%)。詳細 `evals/lessons/20260604_nutritionverse_real_eyelevel_eval_pro_calorie_edge_not_robust.md` / `20260603_n5k_pro_advantage_test_retest_borderline.md`。**採用根拠から frozen-50 含む全 calorie 数値を外し、recognition + 実 mozu データでの最終判断に委ねる**。
