...
ThreadPoolExecutorで100スレッド並列実行
...
def run_parallel_evaluation():
    ...
    print("=" * 80, flush=True)
    print("Parallel Query Evaluation (300 queries, 100 parallel workers)", flush=True)
    print("=" * 80, flush=True)
    ...
    print(f"Starting parallel execution with 100 workers...", flush=True)
    print("=" * 80, flush=True)
        ...
        for future in as_completed(future_to_query):
            ...
            try:
                ...
                if completed % 50 == 0 or completed == len(all_queries):
                    ...
                    print(f"Progress: {completed}/{len(all_queries)} ({completed/len(all_queries)*100:.1f}%) - {rate:.1f} queries/sec", flush=True)

            except Exception as e:
                print(f"❌ Error processing query {query_idx}: {e}", flush=True)
                completed += 1
    ...
    print(f"   Throughput: {len(all_results)/elapsed:.1f} queries/second", flush=True)
    ...
    output_file = output_dir / "evaluation_results_300queries.json"
